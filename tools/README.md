...
It can also prints out any potential problems it finds in a separate column, which not included in the score. This
currently only include some UDFs. The tool won't catch all UDFs, and some of the UDFs can be handled with additional steps. 
...
the rankings of the applications. The output will go into your default filesystem, it supports local filesystem
