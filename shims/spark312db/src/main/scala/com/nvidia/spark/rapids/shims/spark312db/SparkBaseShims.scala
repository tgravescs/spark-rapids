...
import org.apache.spark.sql.rapids.execution.python.shims.spark312db._
...
import org.apache.spark.sql.rapids.shims.spark312db._
...
abstract class SparkBaseShims extends Spark30XShims {
  ...
  override def getGpuColumnarToRowTransition(plan: SparkPlan,
    ...
    if (serClass == classOf[ParquetCachedBatchSerializer]) {
      org.apache.spark.sql.rapids.shims.spark312db.GpuColumnarToRowTransitionExec(plan)
    } else {
