...
class CustomThreadPoolExecutor(corePoolSize: Int,
  ...
  private val taskWaiting = new scala.collection.mutable.HashMap[Long, scala.collection.mutable.Queue[Runnable]]()
...
class MultiFileCloudParquetPartitionReader(
  ...
  private def initAndStartReaders(): Unit = {
    ...
    logWarning(s"init and Start readers task ${TaskContext.get().taskAttemptId()} number files ${files.length} max $maxNumFileProcessed")
    for (i <- 0 until limit) {
      val file = files(i)
