...
case class GpuParquetMultiFilePartitionReaderFactory(
  ...
  override def buildBaseColumnarReaderForCloud(
    ...
    val filterFunc = (file: PartitionedFile, inputFile: InputFile) => {
      filterHandler.filterBlocksInputFile(footerReadType, file, inputFile, conf,
  ...
  private def filterBlocksForCoalescingReader(
    ...
    try {
      logWarning(s"Filtering blocks for coalescing reader, file: ${file.filePath}")
...
class MultiFileParquetPartitionReader(
  ...
  class ParquetCopyBlocksRunner(
    ...
    val createdTime = System.nanoTime()
