...
<a name="sql.format.parquet.multiThreadedRead.maxNumFileParallel"></a>spark.rapids.sql.format.parquet.multiThreadedRead.maxNumFileParallel|A limit on the maximum number of files per task processed in parallel on the CPU side before the file is sent to the GPU. This affects the amount of host memory used when reading the files in parallel.|2147483647
